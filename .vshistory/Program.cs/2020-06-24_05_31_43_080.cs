...
namespace BeatSaberQuestSync
{
    internal class Program
    {
        ...
        private static DirectoryInfo PCDir = new DirectoryInfo(@"G:\Steam\steamapps\common\Beat Saber"/*Environment.CurrentDirectory*/).Combine("Beat Saber_Data", "CustomLevels");
    ...
    internal class ZIP
    {
        public void CreateSample(FileInfo outPathname, string password, string folderName)
        {
            Directory.CreateDirectory(outPathname.DirectoryName);
            using (FileStream fsOut = File.Create(outPathname.FullName))
